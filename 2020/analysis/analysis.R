...
setwd("~/Desktop/Projects/Organizations/G7 and G20 Research Groups/G7 Commitment Analysis")
...
# Convert data to long format
df_commitments <-
  gather(df_commitments, year, commitments,X1975:X2020, factor_key=TRUE)
...
#### G) ANALYZE COMPLIANCE BY OFFICIAL LEVEL BODY CREATION ####

# Read in official body counts
df_official.bodies <- read.csv("official_bodies.csv")

# Merge official body counts with compliance data
df_compliance <- merge(
  df_compliance, df_official.bodies, all.x = T, by = c("area", "year")) %>%
  # Replace missing values with 0
  mutate(bodies = ifelse(is.na(bodies), 0, bodies))

# Test effect of number of official bodies formed on compliance
fit <- polr(score ~ bodies + year, data=df_compliance, Hess=TRUE)
ctable <- coef(summary(fit))
p <- pnorm(abs(ctable[, "t value"]), lower.tail = FALSE) * 2
ctable <- cbind(ctable, "p value" = p)
ctable
...
# Convert data to long format
df_gdp <- gather(df_gdp, year, gdp, X1960:X2020, factor_key=TRUE)
df_gdp.per.cap <-
  gather(df_gdp.per.cap, year, gdp.per.cap, X1960:X2020, factor_key=TRUE)
df_gdp.growth <-
  gather(df_gdp.growth, year, gdp.growth, X1960:X2020, factor_key=TRUE)
...
fit <- polr(score ~ year + min.before + mentions.money + mentions.values +
              ...
              is.ict + is.labor + is.macro + is.security + is.trade + bodies +
              host + gdp.per.cap + gdp.growth,
...
write.csv(df_compliance, "G7 Dataset with Full Features.csv")